...
async function resolveAuthLecturerId(req: Request): Promise<number | null> {
    const userId   = (req as AuthRequest).userId;
    const userRole = (req as AuthRequest).userRole;

    if (!userId || userRole !== "lecturer") return null;
    ...
    const lecRow = await pool.query("SELECT id FROM lecturers WHERE id = $1", [linked]);
    if (lecRow.rows.length === 0) return null;

    return linked as number;
}
...
export async function getLecturerCourses(req: Request, res: Response): Promise<void> {
    try {
...
export async function getLecturerSessions(req: Request, res: Response): Promise<void> {
    try {
        const authLecturerId = await resolveAuthLecturerId(req);
        if (!authLecturerId) {
            res.status(403).json({ message: "Access denied. Not a valid lecturer account." });
...
export async function getLecturerDashboard(req: Request, res: Response): Promise<void> {
    try {
        const authLecturerId = await resolveAuthLecturerId(req);
        if (!authLecturerId) {
            res.status(403).json({ message: "Access denied. Not a valid lecturer account." });
        ...
        const presentTodayResult = await pool.query(
            `SELECT COUNT(a.id)::int AS students_present_today
             FROM attendance a
        ...
        const courseCountResult = await pool.query(
            `SELECT COUNT(*)::int AS total_courses FROM courses WHERE lecturer_id = $1`,
            [authLecturerId]
        );
...
export async function createSessionWithOwnershipCheck(req: Request, res: Response): Promise<void> {
    try {
        ...
        if (userRole === "lecturer") {
            ...
            const courseCheck = await pool.query(
                "SELECT id, lecturer_id FROM courses WHERE id = $1",
                [course_id]
            );
        ...
        const result = await pool.query(
            `INSERT INTO sessions
             ...
             RETURNING *`,
            [course_id, session_date, start_time, end_time, randomUUID(), presentWindow]
        );
...
export async function generateQrWithOwnershipCheck(req: Request, res: Response): Promise<void> {
    try {
        ...
        const sessionResult = await pool.query(
            "SELECT s.*, c.lecturer_id FROM sessions s JOIN courses c ON s.course_id = c.id WHERE s.id = $1",
            [sessionId]
        );
...
export async function closeSessionWithOwnershipCheck(req: Request, res: Response): Promise<void> {
    try {
        const sessionId  = parseInt(String(req.params.id), 10);
        const userRole   = (req as AuthRequest).userRole;
...
export async function getSessionRecordsWithOwnershipCheck(req: Request, res: Response): Promise<void> {
    try {
...
async function verifyLecturerCourse(req: Request, courseId: number): Promise<number | null> {
...
export async function getLecturerCourseReport(req: Request, res: Response): Promise<void> {
    try {
        const courseId = parseInt(String(req.params.courseId), 10);
        if (isNaN(courseId)) { res.status(400).json({ message: "Invalid course ID." }); return; }
        ...
        if (!lecturerId || requestedLecturerId !== lecturerId) { res.status(403).json({ message: "Access denied. You can only report on your own courses." }); return; }
        ...
        const result = await pool.query(
            `SELECT st.id AS student_id, st.student_number, st.full_name, st.programme,
                    COUNT(DISTINCT s.id)::int AS total_sessions,
...
export async function getLecturerStudentHistory(req: Request, res: Response): Promise<void> {
    try {
        const courseId = parseInt(String(req.params.courseId), 10);
        const studentId = parseInt(String(req.params.studentId), 10);
        if (isNaN(courseId) || isNaN(studentId)) { res.status(400).json({ message: "Invalid course or student ID." }); return; }
        const requestedLecturerId = parseInt(String(req.params.lecturerId), 10);
        const lecturerId = await verifyLecturerCourse(req, courseId);
        if (!lecturerId || requestedLecturerId !== lecturerId) { res.status(403).json({ message: "Access denied. You can only view your own course data." }); return; }

        const enrolled = await pool.query(
            "SELECT 1 FROM student_courses WHERE student_id = $1 AND course_id = $2",
            [studentId, courseId]
        );
        if (enrolled.rows.length === 0) { res.status(403).json({ message: "Student is not enrolled in this course." }); return; }
...
export async function getLecturerInsights(req: Request, res: Response): Promise<void> {
    try {
    console.log("ENTER getLecturerInsights", { params: req.params });
        ...
        const courseRatesResult = await pool.query(
            `SELECT
                c.course_code,
                c.course_name,
                c.programme,
