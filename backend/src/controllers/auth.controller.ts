...
export async function register(req: Request, res: Response): Promise<void> {
    try {
        const { email, name, role, linked_id } = req.body as {
            email?: string; name?: string; role?: string; linked_id?: number;
        };
        ...
        const insertResult = await pool.query(
            ...
            [normalizedEmail, name.trim(), role, linked_id ?? null]
        );
...
export async function requestActivation(req: Request, res: Response): Promise<void> {
    try {
        ...
        const result     = await pool.query("SELECT * FROM users WHERE email = $1", [normalised]);
        ...
        try {
            await sendActivationOtpEmail(user.email, user.name, user.role, plainCode);
        } catch (emailErr: any) {
...
export async function verifyActivationOtp(req: Request, res: Response): Promise<void> {
    try {
        ...
        if (ageMins > 30) {
            await pool.query(
                "UPDATE users SET verification_code = NULL, verification_sent_at = NULL, updated_at = NOW() WHERE id = $1",
                [user.id]
            );
            res.status(400).json({
        ...
        const stored = (user.verification_code as string).trim();
        if (code.trim() !== stored) {
...
export async function activateAccount(req: Request, res: Response): Promise<void> {
    try {
        const { email, activation_token, new_password } = req.body as {
            email?:             string;
        ...
        if (new Date() > new Date(user.reset_token_expires)) {
            await pool.query(
                "UPDATE users SET reset_token_hash = NULL, reset_token_expires = NULL, updated_at = NOW() WHERE id = $1",
                [user.id]
            );
            res.status(400).json({
        ...
        const tokenMatch = await bcrypt.compare(activation_token, user.reset_token_hash);
        if (!tokenMatch) {
...
export async function getMe(req: Request, res: Response): Promise<void> {
    try {
        ...
        if (!userId) { res.status(401).json({ message: "Unauthorised." }); return; }
...
export async function requestPasswordReset(req: Request, res: Response): Promise<void> {
    try {
        ...
        let emailWarning: string | undefined;
        try {
            await sendPasswordResetEmail(user.email, user.name, plainCode);
        } catch (emailErr: any) {
...
export async function verifyResetOtp(req: Request, res: Response): Promise<void> {
    try {
        ...
        const tokenHash   = await bcrypt.hash(plainToken, 8); // fast hash; not a password
...
export async function resetPassword(req: Request, res: Response): Promise<void> {
    try {
        const { email, reset_token, new_password } = req.body as {
            email?:        string;
            reset_token?:  string;
            new_password?: string;
        };
        ...
        const tokenMatch = await bcrypt.compare(reset_token, user.reset_token_hash);
        if (!tokenMatch) {
...
export async function updateProfile(req: Request, res: Response): Promise<void> {
    try {
        ...
        const { full_name, department } = req.body as { full_name?: string; department?: string };
        ...
        const userRes = await pool.query("SELECT id, role, linked_id FROM users WHERE id = $1 AND is_active = TRUE", [userId]);
        ...
        if (user.role === "student") {
            ...
            try {
                ...
                const studentRes = await client.query(
                    "UPDATE students SET full_name = $1 WHERE id = $2 RETURNING id, student_number, full_name, email, programme",
                    [value, studentId]
                );
        ...
        const updates: Array<{ sql: string; value: any }> = [];
        if (full_name !== undefined) {
            const v = String(full_name).trim();
            if (v.length === 0) { res.status(400).json({ message: "full_name cannot be empty." }); return; }
            updates.push({ sql: "full_name", value: v });
        }
        if (department !== undefined) {
            const v = String(department).trim();
            if (v.length === 0) { res.status(400).json({ message: "department cannot be empty." }); return; }
        ...
        try {
            ...
            if (updates.length > 0) {
                ...
                const lecRes = await client.query(lecturerUpdateSql, values as any[]);
...
export async function contactAdmin(req: Request, res: Response): Promise<void> {
    try {
        ...
        try {
            await sendContactAdminEmail({ senderName, senderEmail, message: message.trim() });
        } catch (emailErr: any) {
