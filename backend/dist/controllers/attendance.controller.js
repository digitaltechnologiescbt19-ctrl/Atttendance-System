...
const markAttendance = async (req, res) => {
    try {
        ...
        if (callerRole === "student") {
            ...
            if (!callerUserId)
                return res.status(403).json({ message: "Access denied" });
            const userRow = await db_1.default.query("SELECT linked_id FROM users WHERE id = $1 AND role = 'student' AND is_active = TRUE", [callerUserId]);
...
const getStudentAttendance = async (req, res) => {
    try {
        ...
        if (authReq.userRole === "student") {
            const userResult = await db_1.default.query("SELECT linked_id FROM users WHERE id = $1 AND role = 'student' AND is_active = TRUE", [authReq.userId]);
...
const getStudentReport = async (req, res) => {
    try {
        ...
        if (!studentId)
            return res.status(403).json({ message: "No linked student profile found." });
        ...
        const records = await db_1.default.query(`SELECT s.id AS session_id, s.session_date, s.start_time, s.end_time,
             ...
             WHERE ${where}
             ORDER BY s.session_date DESC, s.start_time DESC`, params);
        const courseBreakdown = await db_1.default.query(`SELECT c.id AS course_id, c.course_code, c.course_name,
